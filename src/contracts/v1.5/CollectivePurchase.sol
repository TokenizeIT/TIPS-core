...
{
	function fractionalize(address _target, uint256 _tokenId, string memory _name, string memory _symbol, uint8 _decimals, uint256 _tipsCount, uint256 _tipPrice, address _paymentToken, uint256 _kickoff, uint256 _duration, uint256 _fee) external returns (address _tips);
}
...
contract CollectivePurchase is ERC721Holder, Ownable, ReentrancyGuard
{
	...
	uint8 constant public TIPS_DECIMALS = 6;
	uint256 constant public TIPS_COUNT = 100000e6;
	...
	function buyerTipsCount(uint256 _listingId, address _buyer) external view inState(_listingId, State.Ended) returns (uint256 _tipsCount)
	{
		ListingInfo storage _listing = listings[_listingId];
		uint256 _amount = _listing.buyers[_buyer].amount;
		_tipsCount = (_amount * _listing.tipsCount) / _listing.reservePrice;
		return _tipsCount;
	}

	function sellerPayout(uint256 _listingId) external view returns (uint256 _netAmount, uint256 _feeAmount, uint256 _netTipsCount, uint256 _feeTipsCount)
	{
		ListingInfo storage _listing = listings[_listingId];
		...
		if (_reservePrice > _amount) {
			...
			uint256 _tipsCount = _issuing(_listing.extra);
			_feeTipsCount = _tipsCount * _missingFeeAmount / _reservePrice;
			_netTipsCount = _tipsCount * _missingNetAmount / _reservePrice;
		}
	}
	...
	function relist(uint256 _listingId) public nonReentrant inState(_listingId, State.Started)
	{
		ListingInfo storage _listing = listings[_listingId];
		require(now > _listing.cutoff, "not available");
		uint256 _tipPrice = (_listing.reservePrice + (TIPS_COUNT - 1)) / TIPS_COUNT;
		uint256 _relistTipPrice = (_listing.priceMultiplier * _tipPrice + 99) / 100;
		_listing.state = State.Ended;
		_listing.tips = _fractionalize(_listingId, _relistTipPrice);
		_listing.tipsCount = _balanceOf(_listing.tips);
		items[_listing.collection][_listing.tokenId] = false;
		balances[_listing.tips] = _listing.tipsCount;
	...
	function claim(uint256 _listingId, address payable _buyer) public nonReentrant inState(_listingId, State.Ended)
	{
		...
		uint256 _amount = _listing.buyers[_buyer].amount;
		require(_amount > 0, "insufficient balance");
		uint256 _tipsCount = (_amount * _listing.tipsCount) / _listing.reservePrice;
		_listing.buyers[_buyer].amount = 0;
		balances[_listing.tips] -= _tipsCount;
		_safeTransfer(_listing.tips, _buyer, _tipsCount);
		emit Claim(_listingId, _buyer, _amount, _tipsCount);
	}
	...
	function _issuing(bytes storage _extra) internal pure returns (uint256 _tipsCount)
	{
		(,,,, uint256 _fee) = abi.decode(_extra, (bytes32, string, string, uint256, uint256));
		return TIPS_COUNT - (TIPS_COUNT * _fee / 1e18);
	}

	function _fractionalize(uint256 _listingId, uint256 _tipPrice) internal returns (address _tips)
	{
		ListingInfo storage _listing = listings[_listingId];
		...
		return IAuctionFractionalizer(fractionalizers[_type]).fractionalize(_listing.collection, _listing.tokenId, _name, _symbol, TIPS_DECIMALS, TIPS_COUNT, _tipPrice, _listing.paymentToken, 0, _duration, _fee);
	}
	...
	event Claim(uint256 indexed _listingId, address indexed _buyer, uint256 _amount, uint256 _tipsCount);
}
